...
WHERE D_NAME = 'Smith' AND VDATE = '3-5-88' 
/*或*/
SELECT P#
FROM Visits
WHERE VDATE = '3-5-88' AND D# IN (SELECT D# FROM Doctor WHERE D_NAME = 'Smith')
...
WHERE P# IN 
(
	SELECT P# FROM visit WHERE V_DATE ='3-5-88' AND D# IN 
	(
		SELECT D# FROM Doctor WHERE D_NAME = 'Smith'
	) 
)
...
SELECT MEDICINE
FROM Prescriptions
WHERE P# IN ( SELECT P# FROM Patient WHERE P_NAME = 'Giles' )